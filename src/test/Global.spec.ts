...
describe("Project package util", () => {
    ...
    it("Should find parent result", () => {
        expect(ParentFileFinder.findFile(setTo, "package.json", 1).result.indexOf("/tsmodule-alias/src/test/fake/fakemodule") > -1).to.be.true;
    });
    
    it("should resolve package root", ()=>{
        expect((Package.findImplementorRoot(setTo).result).indexOf("/tsmodule-alias/src/test/fake/fakemodule") > -1).to.be.true;
    });
